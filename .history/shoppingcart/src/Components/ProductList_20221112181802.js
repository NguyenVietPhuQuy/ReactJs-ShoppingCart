...
class ProductList extends Component {
    ...
    calSum =()=>{
        return this.props.cart.reduce((total,item)=>{return total+ item.quantity},0)
    }
    renderProduct() {
        const productFilter = this.props.productList.filter((item)=>item.type === this.props.selected)
        console.log(this.props)
        console.log("productFilter", productFilter)
        const html = productFilter.map((item) => {
            return (
    ...
    render() {
        return (
            <div className='row mt-3 '>
                <h3 onClick={this.showCart} style={{ textAlign: "right", padding: 10, position:"absolute", top:-70, right:40 ,zIndex:1000}}> Cart {this.calSum()}</h3>
                {this.renderProduct()}
