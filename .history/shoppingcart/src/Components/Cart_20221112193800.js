...
class Cart extends Component {
    calcAmount = () => {
        return this.props.cart.reduce((total, item) => { return total + item.quantity * item.prod.price }, 0);
    }
    deleteProduct = (id) => {
    ...
    decreaseProduct = (id) => {
        const cloneCart = [...this.props.cart];
        const foundItem = cloneCart.find(item => id === item.prod.id);
        if (foundItem && cloneCart.quantity>=1) {
            foundItem.quantity -= 1
        }
        this.props.handleDecrease(cloneCart)
                                ...
                                return (
                                    <tr className='text-center align-middle'>
                                        ...
                                        <td className='text-center align-middle d-flex flex-row' style={{ justifyContent: "center", alignItems: "center", marginTop: 27 }}>
                                            <button className='btn btn-info align-middle ' style={{ heigh: 40, width: 40 }} onClick={()=>{this.decreaseProduct(item.prod.id)}}>-</button>
                                        ...
                                        <td className='text-center align-middle'>{item.quantity*item.prod.price}</td>
                                        <td className='text-center align-middle'>
